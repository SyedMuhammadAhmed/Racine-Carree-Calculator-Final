...
base_dir = r"c:\Users\DELL\Desktop\Racine Carree Calculator Final\theme\i18n"
...
for locale in locales:
    with open(os.path.join(locales_dir, f"{locale}.json"), "w", encoding="utf-8") as f:
...
with open(os.path.join(locales_dir, "en.faq.json"), "w", encoding="utf-8") as f:
...
with open(os.path.join(locales_dir, "shared.json"), "w", encoding="utf-8") as f:
    json.dump(shared_data, f, indent=2, ensure_ascii=False)

utils_ts = '''export const LOCALES = ['en', 'hi', 'es', 'ru', 'fr', 'de', 'it', 'pt', 'bn', 'ja', 'ko', 'ms', 'pl', 'id', 'ar', 'bg', 'tr', 'sv'] as const;
...
with open(os.path.join(base_dir, "utils.ts"), "w", encoding="utf-8") as f:
    f.write(utils_ts)

translations_ts = '''import { DEFAULT_LOCALE, type Locale } from './utils';
...
with open(os.path.join(base_dir, "translations.ts"), "w", encoding="utf-8") as f:
    f.write(translations_ts)
